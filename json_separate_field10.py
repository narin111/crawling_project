...
for one in data: # sidosgg.json 추출
    ...
    jarray = j.get("kinderInfo")
    for list in jarray:
        kindername_deduc = list.get("kindername")
        officeedu = list.get("officeedu") # 교육청명
        subofficeedu = list.get("subofficeedu") # 교육지원청명
        kindercode = list.get("kindercode") # 유치원코드
        estb_pt = list.get("estb_pt") # 설립유형
        ...
        deduc_dict = {
            "kindername_deduc" : kindername_deduc,
            "school_ds_yn" : school_ds_yn,
            "school_ds_en" : school_ds_en,
            "educate_ds_yn" : educate_ds_yn,
            "ducate_ds_en" : ducate_ds_en,
            "pbntTmng" : pbntTmng
        }
